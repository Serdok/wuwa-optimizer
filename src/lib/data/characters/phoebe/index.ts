...
const data = (rank: number) => {
	...
	// normal
	builder = builder.set_skill_key('normal', 'o_come_divine_light')
		.set_skill_motions('normal', normal(rank))
		.set_skill_effect('normal', (stats) => {
			if (rank >= 4) stats.enemy_resistance -= 0.1;
		});

	// skill
	builder = builder.set_skill_key('skill', 'to_where_light_shines')
		.set_skill_motions('skill', skill(rank));

	// forte
	builder = builder.set_skill_key('forte', 'radiant_invocation')
		.set_skill_motions('forte', forte(rank))
		.set_motion_effect('forte', 'heavy_attack_starflash_dmg', (stats, { buffs }) => {
	...
	// burst
	builder = builder.set_skill_key('burst', 'dawn_of_enlightenment')
	...
	// intro
	builder = builder.set_skill_key('intro', 'golden_grace')
	...
	// outro
	builder = builder.set_skill_key('outro', 'attentive_heart')
		.set_skill_motions('outro', outro(rank))
		.set_skill_effect('outro', (stats, { buffs }) => {
